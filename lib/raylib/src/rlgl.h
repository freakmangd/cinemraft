...
RLAPI void rlSetVertexAttributeI(unsigned int index, int compSize, int type, int stride, const void *pointer); // Set vertex attribute data configuration
...
// Set vertex attribute
void rlSetVertexAttributeI(unsigned int index, int compSize, int type, int stride, const void *pointer)
{
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
    // NOTE: Data type could be: GL_BYTE, GL_UNSIGNED_BYTE, GL_SHORT, GL_UNSIGNED_SHORT, GL_INT, GL_UNSIGNED_INT
    // Additional types (depends on OpenGL version or extensions): 
    //  - GL_HALF_FLOAT, GL_FLOAT, GL_DOUBLE, GL_FIXED, 
    //  - GL_INT_2_10_10_10_REV, GL_UNSIGNED_INT_2_10_10_10_REV, GL_UNSIGNED_INT_10F_11F_11F_REV
    glVertexAttribIPointer(index, compSize, type, stride, pointer);
#endif
}
